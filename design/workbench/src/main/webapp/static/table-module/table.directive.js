...
angular.module('tableModule').directive('tableItemDirective', function () {
    ...
    function tableController($scope, $http, $attrs, $compile, $element, infoDataService, $timeout, $state, historyUrlService) {
        // 进入该页面，初始化tagInfo
        $scope.first = true;
            ...
            $http({
                ...
                function (data) {
                    ...
                    if (data && data.data && data.data.content.length > 0) {
                        ...
                        $scope.selectedName = '';
                        infoDataService.setSelectedNames('');
                        if ($scope.changePage) {
                            $scope.getAllSelectedTags();
                        }
                    }
