...
angular
	...
	.controller(
	'projectController',
	function ($http, $scope, infoDataService, $state, $timeout, $modal, jobstatusService, $interval, historyUrlService, $window,$rootScope) {
		...
		$scope.getLastModifiedTime = function(){
			return infoDataService.getLastModifiedTime();
		}

		$scope.getCodeVersion = function(){
			return infoDataService.getCodeVersion();
		}

		$scope.refreshCode = function(){
 			$http({
                method: 'POST',
                url: 'http://172.16.17.81:5000/codesearch/'
		...
		$scope.openFileWindow = function () { //open file window
			window.location.href = $scope.sourceCodePath;
		};
		...
		$scope.showJobUpdateStatus = function () { // show job progress bar
			jobstatusService.openJobUpdateStatusModal($scope, infoDataService.getId());
		}
		...
		// $scope.initCodeStatus = function () { // check all required data set status
		// 	$http({
		// 		method: 'GET',
		// 		url: './job/checkstatus',
		// 		params: {
		...
		initProjectStatus();
		function initRemoteSourcePath() { //set remote file path
			...
			$http({
				...
				function (data) {
					$scope.sourceCodePath = data.data.path;
					infoDataService.setCodeVersion(data.data.codeVersion);
					infoDataService.setLastModifiedTime(data.data.lastModifiedTime);
				}).error(
				function (data) {
					$scope.sourceCodePath = infoDataService.getSourcePath();
				});
		};
	...
	.controller('modalCtrl', function ($http, $scope, $modalInstance, projectId, parentscope) {
		$scope.selectedItem = [];
		getAllJobAndDenpend();
